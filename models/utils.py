""" Code from https://github.com/YerevaNN/Dynamic-memory-networks """
...
def load_glove(dim):
    ...
    print "==> loading glove"
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "data/glove/glove.6B/glove.6B." + str(dim) + "d.txt")) as f:
        for line in f:
            l = line.split()
...
def process_word(word, word2vec, vocab, ivocab, word_vector_size, to_return="word2vec", silent=False):
    # if the word isnt in the embedding then create a new embedding for it
